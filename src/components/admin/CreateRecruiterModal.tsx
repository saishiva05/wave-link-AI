...
import { User, Mail, Building, Phone, Lock, Copy, CheckCircle, UserPlus, Loader2, XCircle } from "lucide-react";
...
const CreateRecruiterModal = ({ open, onOpenChange }: CreateRecruiterModalProps) => {
  const [formData, setFormData] = useState({ fullName: "", email: "", company: "", phone: "" });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: "Recruiter created successfully" });
    } catch (err: any) {
      toast({ title: "Error", description: err.message || "Failed to create recruiter", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-[560px] p-0 gap-0 rounded-2xl">
        {success ? (
          <div className="flex flex-col items-center justify-center py-12 px-8 text-center animate-fade-in">
            <div className="w-16 h-16 rounded-full bg-success-50 flex items-center justify-center mb-4"><CheckCircle className="w-8 h-8 text-success-500" /></div>
            <h2 className="text-xl font-bold text-secondary-900 mb-2 font-display">Recruiter Account Created!</h2>
            ...
            <div className="flex gap-3">
              ...
              <Button variant="portal" onClick={() => { setSuccess(false); setFormData({ fullName: "", email: "", company: "", phone: "" }); }}>Create Another</Button>
            </div>
          </div>
        ) : (
          <>
            <DialogHeader className="px-6 pt-6 pb-4 border-b border-border">
              <DialogTitle className="text-xl font-bold text-secondary-900 font-display">Create New Recruiter Account</DialogTitle>
            </DialogHeader>
            <div className="px-6 py-5 space-y-5 max-h-[60vh] overflow-y-auto">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="John Smith" value={formData.fullName} onChange={(e) => updateField("fullName", e.target.value)}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input type="email" placeholder="john.smith@company.com" value={formData.email} onChange={(e) => updateField("email", e.target.value)}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="TechCorp Inc." value={formData.company} onChange={(e) => updateField("company", e.target.value)} className="pl-10" disabled={loading} />
                </div>
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex items-start gap-3">
                <Checkbox id="sendInvite" checked={sendInvite} onCheckedChange={(val) => setSendInvite(val === true)} disabled={loading} />
                <div>
                  <Label htmlFor="sendInvite" className="text-sm text-neutral-700 cursor-pointer">Send login credentials via email</Label>
                  <p className="text-xs text-muted-foreground mt-0.5">The recruiter will receive their login details and a link to set their password.</p>
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">This password will be sent to the recruiter. They can change it after first login.</p>
              </div>
            </div>
            <div className="flex items-center justify-between px-6 py-4 border-t border-border">
              <Button variant="ghost" onClick={handleClose} disabled={loading}>Cancel</Button>
              <Button variant="portal" onClick={handleSubmit} disabled={loading}>
                {loading ? (<><Loader2 className="w-4 h-4 animate-spin" /> Creating...</>) : (<><UserPlus className="w-4 h-4" /> Create Recruiter Account</>)}
              </Button>
            </div>
          </>
        )}

...
import { User, Mail, Lock, Copy, CheckCircle, ShieldPlus, Loader2, XCircle } from "lucide-react";
      ...
      <DialogContent className="sm:max-w-[520px] p-0 gap-0 rounded-2xl">
        {success ? (
          <div className="flex flex-col items-center justify-center py-12 px-8 text-center">
            ...
            <h2 className="text-xl font-bold text-foreground mb-2 font-display">Admin Account Created!</h2>
            <p className="text-sm text-muted-foreground mb-6 max-w-sm">
              {formData.fullName}'s admin account has been created successfully.
            </p>
            <div className="flex gap-3">
              <Button variant="outline" onClick={handleClose}>Close</Button>
              <Button variant="portal" onClick={() => { setSuccess(false); setFormData({ fullName: "", email: "", phone: "" }); }}>Create Another</Button>
            </div>
          </div>
        ) : (
          <>
            <DialogHeader className="px-6 pt-6 pb-4 border-b border-border">
              <DialogTitle className="text-xl font-bold text-foreground font-display">Create New Admin Account</DialogTitle>
            </DialogHeader>
            <div className="px-6 py-5 space-y-5 max-h-[60vh] overflow-y-auto">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="Admin Name" value={formData.fullName} onChange={(e) => updateField("fullName", e.target.value)}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input type="email" placeholder="admin@wavelynk.ai" value={formData.email} onChange={(e) => updateField("email", e.target.value)}
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">Share this password securely with the new admin.</p>
              </div>
            </div>
            <div className="flex items-center justify-between px-6 py-4 border-t border-border">
              <Button variant="ghost" onClick={handleClose} disabled={loading}>Cancel</Button>
              <Button variant="portal" onClick={handleSubmit} disabled={loading}>
                {loading ? (<><Loader2 className="w-4 h-4 animate-spin" /> Creating...</>) : (<><ShieldPlus className="w-4 h-4" /> Create Admin Account</>)}
              </Button>
            </div>
          </>
        )}

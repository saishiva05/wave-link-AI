...
const EditJobModal = ({ open, onOpenChange, job }: EditJobModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.job_title || !form.company_name || !form.location) {
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-xl font-bold font-display">Edit Job Posting</DialogTitle>
              <p className="text-sm text-muted-foreground mt-0.5">Update the job details below</p>
            </div>
          </div>
        </DialogHeader>
